...
void print_random_data()
{
  ...
  const intType person_max = 100;
  const intType person_min = 1;
  Distribution person_distribution(person_min, person_max);
  auto generate_person = [&]() { return person_distribution(engine); };
